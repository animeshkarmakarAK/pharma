        ...
        return DataTables::eloquent($questionAnswers)
            ->addColumn('action', static function (QuestionAnswer $questionAnswer) use ($authUser) {
                ...
                if ($authUser->can('view', $questionAnswer)) {
                    $str .= '<a href="' . route('course_management::admin.question-answers.show', $questionAnswer->id) . '" class="btn btn-outline-info btn-sm"> <i class="fas fa-eye"></i> ' . __('generic.read_button_label') . ' </a>';
                }

                if ($authUser->can('update', $questionAnswer)) {
                    $str .= '<a href="' . route('course_management::admin.question-answers.edit', $questionAnswer->id) . '" class="btn btn-outline-warning btn-sm"> <i class="fas fa-edit"></i> ' . __('generic.edit_button_label') . ' </a>';
                }
                if ($authUser->can('delete', $questionAnswer)) {
                    $str .= '<a href="#" data-action="' . route('course_management::admin.question-answers.destroy', $questionAnswer->id) . '" class="btn btn-outline-danger btn-sm delete"> <i class="fas fa-trash"></i> ' . __('generic.delete_button_label') . '</a>';
                }
