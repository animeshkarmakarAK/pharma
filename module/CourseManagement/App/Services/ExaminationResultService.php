        ...
        return DataTables::eloquent($examinationResults)
            ->addColumn('action', DatatableHelper::getActionButtonBlock(static function (ExaminationResult $examinationResult) use ($authUser) {
                ...
                if ($authUser->can('view', $examinationResult)) {
                    $str .= '<a href="' . route('course_management::admin.examination-results.show', $examinationResult->id) . '" class="btn btn-outline-info btn-sm"> <i class="fas fa-eye"></i> ' . __('generic.read_button_label') . '</a>';
                }
                if ($authUser->can('update', $examinationResult)) {
                    $str .= '<a href="' . route('course_management::admin.examination-results.edit', $examinationResult->id) . '" class="btn btn-outline-warning btn-sm"> <i class="fas fa-edit"></i> ' . __('generic.edit_button_label') . '</a>';
                }
                if ($authUser->can('delete', $examinationResult)) {
                    $str .= '<a href="#" data-action="' . route('course_management::admin.examination-results.destroy', $examinationResult->id) . '" class="btn btn-outline-danger btn-sm delete"> <i class="fas fa-trash"></i> ' . __('generic.delete_button_label') . '</a>';
                }
