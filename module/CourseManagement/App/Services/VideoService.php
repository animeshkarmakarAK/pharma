        ...
        return DataTables::eloquent($videos)
            ->addColumn('action', static function (Video $video) use ($authUser) {
                ...
                if ($authUser->can('view', $video)) {
                    $str .= '<a href="' . route('course_management::admin.videos.show', $video->id) . '" class="btn btn-outline-info btn-sm"> <i class="fas fa-eye"></i> ' . __('generic.read_button_label') . ' </a>';
                }

                if ($authUser->can('update', $video)) {
                    $str .= '<a href="' . route('course_management::admin.videos.edit', $video->id) . '" class="btn btn-outline-warning btn-sm"> <i class="fas fa-edit"></i> ' . __('generic.edit_button_label') . ' </a>';
                }
                if ($authUser->can('delete', $video)) {
                    $str .= '<a href="#" data-action="' . route('course_management::admin.videos.destroy', $video->id) . '" class="btn btn-outline-danger btn-sm delete"> <i class="fas fa-trash"></i> ' . __('generic.delete_button_label') . '</a>';
                }
