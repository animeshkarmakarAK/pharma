        ...
        return DataTables::eloquent($programmes)
            ->addColumn('action', DatatableHelper::getActionButtonBlock(static function (Programme $programme) use ($authUser) {
                ...
                if ($authUser->can('view', $programme)) {
                    $str .= '<a href="' . route('course_management::admin.programmes.show', $programme->id) . '" class="btn btn-outline-info btn-sm"> <i class="fas fa-eye"></i> ' . __('generic.read_button_label') . '</a>';
                }
                if ($authUser->can('update', $programme)) {
                    $str .= '<a href="' . route('course_management::admin.programmes.edit', $programme->id) . '" class="btn btn-outline-warning btn-sm"> <i class="fas fa-edit"></i> ' . __('generic.edit_button_label') . '</a>';
                }
                if ($authUser->can('delete', $programme)) {
                    $str .= '<a href="#" data-action="' . route('course_management::admin.programmes.destroy', $programme->id) . '" class="btn btn-outline-danger btn-sm delete"> <i class="fas fa-trash"></i> ' . __('generic.delete_button_label') . '</a>';
                }
